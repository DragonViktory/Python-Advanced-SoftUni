...
class FoodOrdersApp:
    ...
    @staticmethod
    def decrease_quantity_of_menu(shopping_list, menu) -> None:
        shopping_list_dict = {}

        for meal in shopping_list:
            if meal.name not in shopping_list_dict:
                shopping_list_dict[meal.name] = 0

            shopping_list_dict[meal.name] += meal.quantity

        for meal in menu:
            if meal.name in shopping_list:
                meal.quantity -= shopping_list[meal.name]
